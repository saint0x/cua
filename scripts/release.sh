...
printf 'rlm_provider_host_scenario=%s\n' "$RLM_PROVIDER_HOST_SCENARIO"
...
if [[ "$SKIP_FOZZY" -eq 0 ]]; then
  ...
  if env_key_available OPENROUTER_API_KEY; then
    ...
    log "Skipping Fozzy host-backed cua-rlm-loop-provider-progress-host; OPENROUTER_API_KEY unavailable"
  fi
...
run scripts/host-voice-proof-suite.sh | tee "$ARTIFACT_DIR/host-voice-proof-suite-path.txt"
