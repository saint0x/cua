...
Cloud enrollment is not shipped yet. The current release can sign and verify runtime claims for the selected backend, including `remote_cua` and `oracle_oci` when selected through environment configuration. OCI lifecycle operations are CLI-backed; a launched OCI instance becomes controllable only after it exposes a CUA HTTP endpoint and token. Quilt/cloud enrollment persistence and revocation are still outside the shipped attestation surface.
